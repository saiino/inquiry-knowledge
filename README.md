...
このリポジトリには、AIによる破壊的操作を防ぐ共有ガードが入っている。**クローンすれば全員に自動で適用される。**

| ファイル | 役割 |
|---|---|
| `.claude/settings.json` | 削除系コマンド(`rm` 等)・force push などを**機械的にブロック**する拒否設定 |
| `CLAUDE.md` | 毎セッション自動で読み込まれる安全ルール(リポジトリ外を触らない、削除はしない、等) |

担当者側の約束は3つだけ:

1. `--dangerously-skip-permissions` オプションを使わない(名前の通り、上記ガードごと無効になる)
